...
# Вход: экземпляр диспетчера
# Выход: регистрация функций как хендлеров
async def register_handlers(dp: Dispatcher) -> None:
    dp.message.register(start, Command("start"))
    dp.message.register(register, Command("register"))
    dp.message.register(process_registration, RegisterState.waiting_for_credentials)
    dp.message.register(handle_simulate_command, Command("simulate"))
    dp.message.register(run_simulation, SimulateState.waiting_for_qubit_number)
    dp.message.register(history, Command("history"))
